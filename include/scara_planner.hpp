...
private:
    ...
    // place->place_safe->pick_safe->pick->pick_safe->place_safe->place
    std::array<std::array<double, 4>, 7> _lu_path{}; // ！< 一共有7个waypoint，每个waypoint对应4个关节角度的目标值
    // place->tighten->place
    std::array<std::array<double, 4>, 3> _trt_path{}; // ！< 一共有3个waypoint，每个waypoint对应4个关节角度的目标值
    size_t _seg_idx{0};                               //!< 当前执行到第几段轨迹
    // TaskMode _task_mode{TaskMode::Idle};              //!< 当前任务模式
    std::atomic<TaskMode> _task_mode{TaskMode::Idle}; //!< 当前任务模式,把 _task_mode 改成原子变量
};