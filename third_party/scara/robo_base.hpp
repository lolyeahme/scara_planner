...
#define LZ_RES      SCREW_LEAD       //单位设定，coordination输入量转motionmap
/////////////////////////////////// 编码特值///////////////////////////////////
...
struct MotorInfo {
    ...
    // constexpr 构造 编译期初始化
    constexpr MotorInfo()
        : positionRes   {RX_POSITION_RES, RY_POSITION_RES, ZU_POSITION_RES, ZP_POSITION_RES}
        , homePos       {RX_HOME_POSITION, RY_HOME_POSITION, ZU_HOME_POSITION, ZP_HOME_POSITION}
        , maxLimit      {RX_MAX_POSITION, RY_MAX_POSITION, ZU_MAX_POSITION, ZP_MAX_POSITION}
        , minLimit      {RX_MIN_POSITION, RY_MIN_POSITION, ZU_MIN_POSITION, ZP_MIN_POSITION}
        , zeroPos       {RX_ZERO_POSITION, RY_ZERO_POSITION, ZU_ZERO_POSITION, ZP_ZERO_POSITION}
        , direction     {RX_Direction, RY_Direction, ZU_Direction, ZP_Direction}
        , velocity      {RX_VELOCITY, RY_VELOCITY, ZU_VELOCITY, ZP_VELOCITY}
    {}
...
public:
    RoBoMotion()
        ...
        : statepoints(toInt(AxisIndex::COUNT)), sampleFreqs(toInt(AxisIndex::COUNT))
        {
    ...
    struct ScrewMotion
    {
        ...
        {
            u_p = std::abs(distance) / lead * PI;  // lead为导程包含方向 运算得到旋转角度
    ...
    struct MDH_Link {
        double alpha;       // 扭转角 (度)
        double a;           // 杆长 (mm)
        double d_static;    // d 的静态部分 (mm)
        double theta_static;// theta 的静态部分 (度)
        bool is_prismatic;  // true=移动关节, false=旋转关节
    };
    static inline MotorInfo motorinfo;
     ...
     * @param  pulse[3] z
     */
    static Coordination    forword_kinematics_pulseToUnit(const std::vector<int32_t>  &pulse);
    static Coordination    forword_kinematics(const std::vector<double> &unit);
     ...
     * @param u_guess 初始猜测解
     * @return std::vector<double> 逆运动学解弧度
     */
    std::vector<double> solve_dls_high_precision(const Coordination& target_pose, const std::vector<double>& u_guess);
    /**
     * @brief 逆运动学 单位值转脉冲
     * 
...
private:
    static inline double L1 = L1_LENGTH;
    static inline double L2 = L2_LENGTH;
    static inline double LZ = LZ_RES;

    // 链接参数
    // static inline std::vector<MDH_Link> links = {
    //     { -0.001778, -0.131815, 0.037434, 0.012929, false },
    //     { -0.000082, 249.386365, 0.037434, -0.004170, false },
    //     { -0.307177, 250.655862, 0.037434, -0.029453, false },
    //     { 0.289244, 0.666879, 0.037434, -0.029453, true }
    // };
    static inline std::vector<MDH_Link> links = {
        { -0.0, -0.0, 0.0, 0.0, false },
        { -0.0, 250, 0.0, -0.0, false },
        { -0.0, 250, 0.0, -0.0, false },
